...
#ifndef ParisSingleHit_h
#define ParisSingleHit_h 1
...
/*!   
	...
	the list of the available informations for each ParisSingleHit. Use Get/Set methods to obtain/change these values.
...
*/
class ParisSingleHit : public G4VHit
{
public:
   ParisSingleHit();
   ~ParisSingleHit();
   ParisSingleHit(const ParisSingleHit &right);
   
   const ParisSingleHit& operator=(const ParisSingleHit &right);
   G4int operator==(const ParisSingleHit &right) const;
...
public:
   ...
   void SetPDGcode(G4int code) { PDGcode = code;}
   ...
   G4int GetMotherID(){ return motherID; }    
   ...
   G4String &GetMotherDetName() {  return motherDetName; }
};

typedef G4THitsCollection<ParisSingleHit> ParisSingleHitsCollection;

extern G4Allocator<ParisSingleHit> ParisSingleHitAllocator;

inline void* ParisSingleHit::operator new(size_t)
{
  void *aHit;
  aHit = (void *) ParisSingleHitAllocator.MallocSingle();
  return aHit;
}

inline void ParisSingleHit::operator delete(void *aHit)
{
  ParisSingleHitAllocator.FreeSingle((ParisSingleHit*) aHit);
}
