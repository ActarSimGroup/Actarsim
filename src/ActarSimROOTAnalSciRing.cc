...
ActarSimROOTAnalSciRing::ActarSimROOTAnalSciRing() {
  ...
  //The clones array of SciRing Hits
...
void ActarSimROOTAnalSciRing::FillingHits(const G4Event *anEvent) {
  ...
  ActarSimSciRingGeantHitsCollection* hitsCollection =
    (ActarSimSciRingGeantHitsCollection*) HCofEvent->GetHC(hitsCollectionID);
  ...
  for (G4int i=0;i<NbHits;i++) {
    ...
    if(hitsCounter==0){ //only for the first geantHit accepted for storage
      ...
      AddCalCrystalHit(theSciRingHit[hitsCounter],(*hitsCollection)[i],0);
	...
	if( (*hitsCollection)[i]->GetDetName() == name[j] &&
	  ...
	  AddCalCrystalHit(theSciRingHit[j],(*hitsCollection)[i],1);
  ...
  delete [] detID;
  delete [] name;
  for (G4int i=0;i<NbCrystalsWithHit;i++) delete theSciRingHit[i];
