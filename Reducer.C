...
//      .L Reducer.C;
//      Run("simFile","digFile","outputFile")
...
void Run(char* simname, char* digname, char* outname)
{
  ...
  //simname=TString::Format("./root_files/sim_files/%s",simname);
  ...
  thePadsGeometry.SetGeometryValues(0.,0.,0.,37.,85.,69.,0.,0.,0.,2.,5.,5.);
  ...
  */
  ...
  //digname=TString::Format("./root_files/dig_files/%s",digname);

  TFile *digFile=new TFile(digname);
  //TFile *digFile = new TFile(TString::Format("./root_files/dig_files/%s",digname),"RECREATE");
  ...
  //outname=TString::Format("./root_files/out_files/%s",outname);
  TFile *outfile = new TFile(outname,"RECREATE");
  //TFile *outfile = new TFile("./root_files/out_files/Output.root","RECREATE");
  //TFile *outfile = new TFile(TString::Format("./root_files/out_files/%s",outname),"RECREATE");
  cout<<"Writing in output file: "<<outname<<endl;
  ...
  TTree *out_tree = new TTree("out_tree","out_tree");  
   ...
   TMatrixD padChargeMat;
   TMatrixD padTimeMat;

   TMatrixD a(numberOfRows,numberOfColumns);
   padChargeMat.ResizeTo(a);
   padTimeMat.ResizeTo(a);

   TMatrixD *pointerCharge = &padChargeMat;
   TMatrixD *pointerTime = &padTimeMat;
   ...
   //ReadDig("./dig_files/digFile_alpha_z80_ionGas.root", "isobutane",padChargeMat,padTimeMat,i);
   ...
   for (Long64_t jentry=0;jentry<dentries;jentry++) {
     //for (Long64_t jentry=50000;jentry<nentries;jentry++) {
     //for (Long64_t jentry=49500;jentry<nentries;jentry++) {
     ...
     //digiTree->GetEvent(jentry-50000);//to get the 2nd part of events
     //digiTree->GetEvent(jentry-50000);//to get the 2nd part of events
	 ...
	 if (thisCharge!=0){
	   ...
	   padChargeMat(padSignal->GetPadRow()-1,padSignal->GetPadColumn()-1)+= thisCharge;
	   padTimeMat(padSignal->GetPadRow()-1,padSignal->GetPadColumn()-1)+= thisCharge*thisTime;
	   ...
	   //if(numberofpads>2096)cout<<"numberofpads "<<numberofpads<<" Row "<<padSignal->GetPadRow()-1<<" Col "<<padSignal->GetPadColumn()-1<<" Charge "<<thisCharge<<endl;
	 }
       }//Loop on ActarPadSignals
	 

       if(jentry%beamOn==(beamOn-1))//To have the 2 parts of the reaction
	 {
	   // Loop on rows & columns
	   for(Int_t c=0;c<numberOfColumns;c++){ //LOOP on Columns
	     for(Int_t r=0;r<numberOfRows;r++){ //LOOP on Rows
	       padTimeMat(r,c)=padTimeMat(r,c)/padChargeMat(r,c);
	   ...
	   if(nbsiliconhits>0){
	     ...
	     for(Int_t h=0;h<nbsiliconhits;h++){
	       ...
	       else SilID(detectorID-1)=-1;//unknown
	       //break;
